...
1. **状态分组逻辑** (`src/nodes/condition/condition-inputs/index.tsx`)
   - 按状态ID (`_stateId`) 对条件进行分组
   - 创建 `StateGroup` 组件提供状态标识
   - 保持原有条件编辑和删除功能
...
3. **状态名称友好显示**
   - 自动从状态ID提取显示名称
   - 例如：`Vehicle.dumperAction_state` → `dumperAction`
...
#### 相关文档
- `docs/components/condition-node-state-grouping.md` - 专门文档
- `docs/components/enhanced-input-components.md` - 更新记录
